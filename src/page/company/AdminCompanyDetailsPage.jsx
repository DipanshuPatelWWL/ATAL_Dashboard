...
const CompanyDetailsPage = () => {
    ...
    if (!company) return <div className="p-6 text-center">Loading company details...</div>;
    ...
    const renderFileLink = (file) => {
        ...
        const fileUrl = fileName.startsWith("http://") || fileName.startsWith("https://") ? fileName : `${IMAGE_URL}${fileName}`;
        return (
            <a
                href={fileUrl}
                target="_blank"
    ...
    // Define fields
    const importantFields = ["companyName", "registrationNumber", "providerNumber"];
    const companyFields = [
        "companyEmail",
        "serviceStandards",
        "providerEmail",
    ...
    const otherFields = Object.keys(company).filter((key) => !knownFields.includes(key));
    ...
    return (
        <div className="p-6">
            ...
            {/* Page Title */}
            <h2 className="text-2xl font-bold text-black mb-6 text-center">{company.companyName} — Full Details</h2>

            {/* Important Summary */}
            <div className="bg-white shadow-lg rounded-2xl p-6 border border-gray-200 mb-6">
                <h3 className="text-xl font-semibold text-black mb-4 border-b pb-2">Important Details</h3>
                <div className="grid grid-cols-3 gap-4">
                    {importantFields.map((field) => (
                        ...
                        >
                            ...
                            <p className="text-gray-900 ml-2 break-all">{company[field] || "—"}</p>
                        </div>
                    ))}
                </div>
            </div>
