...
const Login = () => {
  ...
  // Forgot password states
  const [step, setStep] = useState(0); // 0 = hidden, 1 = email, 2 = otp, 3 = reset
  const [resetEmail, setResetEmail] = useState("");
  const [otp, setOtp] = useState("");
  const [newPassword, setNewPassword] = useState("");
  ...
  const sendOtp = async () => {
    try {
      const res = await API.post("/auth/send-otp", { email: resetEmail });
      ...
      <div className="mb-4 mt-4 flex max-w-6xl flex-col sm:mx-2 xl:flex-row">
        ...
        <div className="w-full bg-[#d7e0dd shadow-2xl xl:w-2/3 xl:rounded-bl-2xl xl:rounded-tl-2xl">
          <div className="m-10 max-w-5xl">
            ...
            {step > 0 && (
              <div className="max-w-md mx-auto bg-white p-6 rounded shadow-md mt-4">
                {step === 1 && (
                  <>
